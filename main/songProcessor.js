...
function Request(artSRC) {
    ...
    if(_song != "ad" && _song != "audioad"){
	    ...
	    chrome.extension.sendMessage({greeting: "song-change", art : artSRC, artist : _artist, album : _album, song : _song, thumb : _thumb}, function(response) {
	        console.log("Pandora History - " + response.farewell);
	...
	var polledArt = document.getElementsByClassName("playerBarArt")[0]; // assumes only 1 element of that class on the page
		...
		if(polledArt != null){
			...
			if(polledArtSRC != currentArtSRC){
					console.log("Pandora History - Song Change");			
